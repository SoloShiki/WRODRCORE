...
# ---------------- Live Maze Drawing ----------------
def draw_maze_live(maze, start, goal, current=None, path=None):
    os.system("clear")  # for Linux/macOS, use "cls" on Windows
...
# ---------------- Map Grid Navigation ----------------
def follow_path(node, path, odom_sub, maze, start, goal, cell_size=0.2):
    for i in range(1, len(path)):
        cur = path[i-1]
        nxt = path[i]

        # draw live maze with robot at nxt
        draw_maze_live(maze, start, goal, current=nxt, path=path)
...
def main():
    ...
    # Animate robot following path
    follow_path(node, path, odom_sub=odom_reader, maze=maze, start=start, goal=goal, cell_size=0.2)
